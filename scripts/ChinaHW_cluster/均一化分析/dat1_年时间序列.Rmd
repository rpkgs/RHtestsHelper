...
p2 <- plot_trend_tair(pdat_final[varname == "Tair_avg"], fontsize = fontsize)
...
pdat_final <- filter_homo(pdat2)
# 1290个站点发生过调整
p_rh = plot_trend_tair(pdat_final, lgd_title = lab_trend_RH, fontsize = fontsize, theme = NULL)

p <- patchwork::wrap_plots(p1, p2, p_rh, ncol = 1)
# geom_signPoint(aes(mask = pvalue <= 0.05), shape = 4)
write_fig(p, "d:/trend_RH_2022.pdf", 12, 9)
```
...
d = data_year[site %in% sites_ex] %>% merge(info) %>% 
  mutate(name = factor(name, names_ex), 
